...
func (p *PortForwarder) KeepAlive(ctx context.Context) {
	for {
		select {
		case <-ctx.Done():
			slog.Error("port forwarder: keep alive cancelled", "error", ctx.Err())
			return
		case pfErr := <-p.errChan:
			...
			slog.Error("port forwarder: received error signal", "error", pfErr)
			p.Stop()
			if err := p.Forward(ctx); err != nil {
				slog.Error("port forwarder: could not restart session", "error", err)

				select {
				case <-ctx.Done():
					slog.Info("port forwarder: keep alive cancelled", "error", ctx.Err())
					return
